<!-- ========================================================= -->
<!--                 GITHUB PROFILE README - PART 1             -->
<!-- ========================================================= -->

<div align="center">

<!-- Replace this banner with your own custom banner -->
<img src="https://capsule-render.vercel.app/api?type=waving&height=250&color=0:0f172a,50:1e3a8a,100:38bdf8&text=Nikhil%20Kumar%20Jha&fontColor=ffffff&fontSize=48&fontAlignY=38&desc=AI%20Engineer%20%7C%20Building%20Production-Ready%20Intelligent%20Systems&descAlignY=58&animation=fadeIn" width="100%"/>

<br>

<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=600&size=24&duration=3000&pause=1000&color=38BDF8&center=true&vCenter=true&width=900&lines=AI+Engineer;Multi-Agent+Systems+Developer;Building+Production-Ready+AI+Applications;RAG+%7C+MCP+%7C+LLMs+%7C+Backend+Engineering;Always+Learning.+Always+Shipping." />

<br><br>

<img src="https://komarev.com/ghpvc/?username=nick2726&label=Profile%20Views&color=0e75b6&style=for-the-badge" />

<img src="https://img.shields.io/github/followers/nick2726?label=Followers&style=for-the-badge" />

<img src="https://img.shields.io/github/stars/nick2726?affiliations=OWNER&style=for-the-badge" />

</div>

---

# 👋 Hello, I'm Nikhil Kumar Jha

### AI Engineer • Backend Developer • Problem Solver

I build **production-ready AI systems** that solve real business problems using modern AI engineering practices.

Rather than creating demo projects, I focus on designing scalable applications with clean architecture, autonomous agents, retrieval systems, cloud-native deployment, and developer-friendly APIs.

My goal is to become an engineer who can own an entire AI product—from system design to deployment.

---

# 🚀 Current Focus

```yaml
Currently Building:

  ✅ Enterprise Multi-Agent Financial Research Platform

  ✅ Retrieval-Augmented Generation (RAG) Systems

  ✅ Model Context Protocol (MCP) Applications

  ✅ Production AI APIs using FastAPI

  ✅ Cloud-native AI Deployments

  ✅ Backend Systems for LLM Applications
```

---

# 💫 About Me

```text
🎓 Final Year Computer Science Engineering Student

🤖 Passionate about AI Engineering and Backend Systems

⚙️ Building scalable Multi-Agent AI platforms

📚 Learning Distributed Systems, System Design, and Cloud

🌱 Exploring LLM Infrastructure, RAG, MCP and AI Orchestration

💡 Interested in solving real-world business problems with AI

🎯 Target Role:
Software Development Engineer (AI / Backend)

📍 India
```

---

# 🧠 Engineering Philosophy

> "A great engineer doesn't build the most projects.
> They build systems that people can depend on."

I believe software engineering is not just about writing code.

It is about

- Designing scalable architectures
- Writing maintainable code
- Building production-ready software
- Solving business problems
- Continuously improving developer experience

---

# 🎯 Core Expertise

<table>
<tr>

<td width="33%" align="center">

### 🤖 AI Engineering

Multi-Agent Systems

Retrieval-Augmented Generation

Model Context Protocol

LLM Applications

Prompt Engineering

AI Workflows

</td>

<td width="33%" align="center">

### ⚙️ Backend Engineering

FastAPI

Node.js

Express

REST APIs

Authentication

Microservices

</td>

<td width="33%" align="center">

### ☁️ Cloud & DevOps

Docker

Kubernetes

AWS

Google Cloud

CI/CD

GitHub Actions

</td>

</tr>
</table>

---


# 🚀 Featured Projects

> Building **production-ready AI systems** with a focus on scalability, backend engineering, autonomous agents, and cloud-native deployment.

---

## 🌟 Flagship Project

### 🧠 Enterprise Multi-Agent Financial Research Platform

> A production-oriented AI platform that automates financial research using autonomous AI agents, Retrieval-Augmented Generation (RAG), and enterprise backend architecture.

### ✨ Key Features

* 🤖 Multi-Agent Architecture
* 🧠 Retrieval-Augmented Generation (RAG)
* 🔌 Model Context Protocol (MCP) Ready
* 📈 Financial Statement Analysis
* 📊 Peer Company Comparison
* 📑 SEC Filing Analysis
* ⚡ FastAPI REST APIs
* 🐳 Docker Support
* ☁️ Cloud Deployment Ready
* 📚 Modular & Scalable Codebase

### Tech Stack

`Python` • `FastAPI` • `LangGraph` • `LangChain` • `Gemini` • `PostgreSQL` • `Docker`

<div align="center">

[![Repository](https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/nick2726/multi-agent-financial-research-analyst)

</div>

---

# 🚗 Featured Projects

<table>

<tr>

<td width="50%">

## 🚗 CARVIS AI

AI-powered in-vehicle assistant focused on driver safety using Computer Vision and Artificial Intelligence.

### Highlights

* Driver Monitoring
* Drowsiness Detection
* Emotion Recognition
* Voice Assistant
* Object Detection
* Safety Alerts

**Status**

🚧 Active Development

</td>

<td width="50%">

## 🤖 AI Coding Assistant

Enterprise developer assistant powered by Multi-Agent Systems, MCP, and Retrieval-Augmented Generation.

### Planned Features

* Repository Chat
* AI Code Review
* Documentation Generator
* Pull Request Analysis
* Autonomous Agents

**Status**

📅 Coming Soon

</td>

</tr>

<tr>

<td width="50%">

## 🌐 Developer Portfolio

Modern portfolio showcasing projects, architecture diagrams, blogs, and engineering case studies.

### Features

* Modern UI
* Animated Experience
* Resume
* Blogs
* GitHub Integration
* Live Projects

**Status**

🚧 In Progress

</td>

<td width="50%">

## 🛕 Temple Reservation Platform

Full-stack reservation platform with secure authentication, booking workflows, payment integration, and QR-based verification.

### Highlights

* Authentication
* Booking Engine
* Payment Workflow
* QR Generation
* Responsive UI

</td>

</tr>

</table>

---

# 🏗 Engineering Domains

<div align="center">

|         AI Engineering         | Backend Engineering | Cloud & DevOps |
| :----------------------------: | :-----------------: | :------------: |
|       Multi-Agent Systems      |       FastAPI       |       AWS      |
| Retrieval-Augmented Generation |       Node.js       |  Google Cloud  |
|     Model Context Protocol     |      REST APIs      |     Docker     |
|        LLM Applications        |    Authentication   |   Kubernetes   |
|        AI Orchestration        |    Microservices    | GitHub Actions |
|       Prompt Engineering       | Distributed Systems |      CI/CD     |

</div>

---

# 🔭 Currently Building

```text
🧠 Enterprise Multi-Agent Financial Research Platform

🚗 CARVIS AI

🤖 AI Coding Assistant

🌐 Premium Developer Portfolio

☁️ Kubernetes Deployment Pipeline

📦 Production-Ready AI APIs
```

---

# 📌 Engineering Principles

```text
✔ Clean Architecture

✔ Scalable Backend Design

✔ Modular Development

✔ Production-First Mindset

✔ API-First Development

✔ Cloud Native Deployment

✔ Continuous Learning

✔ Real-World Problem Solving
```

---

<div align="center">

### ⭐ "I build software that solves problems—not just projects that demonstrate syntax."

</div>

---


# 🔭 What I'm Working On

| Project | Description | Status |
|----------|-------------|--------|
| 🧠 Multi-Agent Financial Research Platform | Enterprise AI platform for financial analysis | 🚧 Active |
| 🚗 CARVIS AI | Intelligent in-vehicle AI assistant | 🚧 Active |
| 🤖 AI Coding Assistant | MCP-powered developer assistant | 📅 Planned |
| 🌐 Personal Portfolio | Premium developer portfolio | 🚧 In Progress |

---

# 📈 2026 Goals

- Build enterprise-grade AI products
- Contribute to open source
- Strengthen system design skills
- Master distributed backend systems
- Deploy scalable cloud-native applications
- Secure an AI/Backend Software Engineer role

---

<div align="center">

### 💬 "Building software that creates impact, not just repositories."

</div>

---


## 🌐 Socials:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/https://www.linkedin.com/in/nikhil-kumar-jha-b7650b24b/) [![email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:nikhilnkj27@gmail.com) 

# 💻 Tech Stack:
````md
<!-- ======================================================== -->
<!--                 TECHNOLOGY STACK                         -->
<!-- ======================================================== -->

# 💻 Technology Stack

> Technologies I use to design, develop and deploy production-ready software.

---

# 👨‍💻 Languages

<p align="center">

<img src="https://skillicons.dev/icons?i=python,cpp,java,ts,js,sql" />

</p>

---

# ⚙️ Backend Engineering

<p align="center">

<img src="https://skillicons.dev/icons?i=fastapi,nodejs,express,django,flask" />

</p>

**Core Skills**

- REST APIs
- Authentication & Authorization
- JWT
- API Design
- Microservices
- Backend Architecture
- Scalable Services

---

# 🎨 Frontend

<p align="center">

<img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,redux,html,css" />

</p>

---

# 🤖 Artificial Intelligence

### AI Frameworks

- LangChain
- LangGraph
- MCP (Model Context Protocol)
- OpenAI SDK
- Google Gemini
- Hugging Face Transformers

### Computer Vision

- OpenCV
- YOLO
- TensorFlow
- PyTorch

### Machine Learning

- Scikit-Learn
- NumPy
- Pandas

---

# 🗄 Databases

<p align="center">

<img src="https://skillicons.dev/icons?i=postgres,mongodb,redis,sqlite,supabase" />

</p>

---

# ☁️ Cloud

<p align="center">

<img src="https://skillicons.dev/icons?i=aws,gcp,firebase" />

</p>

---

# 🚀 DevOps

<p align="center">

<img src="https://skillicons.dev/icons?i=docker,kubernetes,jenkins,githubactions" />

</p>

### Deployment Platforms

- Vercel
- Render
- Netlify

---

# 🛠 Developer Tools

<p align="center">

<img src="https://skillicons.dev/icons?i=git,github,linux,postman,vscode,figma" />

</p>

---

# 🧠 Areas of Interest

<div align="center">

| Artificial Intelligence | Software Engineering | Cloud |
|:------------------------:|:-------------------:|:------:|
| Multi-Agent Systems | Backend Engineering | Docker |
| Retrieval-Augmented Generation | System Design | Kubernetes |
| Model Context Protocol | Distributed Systems | AWS |
| AI Agents | API Engineering | Google Cloud |
| LLM Applications | Microservices | CI/CD |

</div>

---

# 📚 Currently Learning

```text
✓ Distributed Systems

✓ Event Driven Architecture

✓ Kubernetes

✓ Cloud Native Development

✓ AI Infrastructure

✓ Large Language Models

✓ Advanced System Design

✓ AI Agent Frameworks
```

---

# 🚀 Development Workflow

```text

          Idea

           │

           ▼

    Problem Analysis

           │

           ▼

     System Design

           │

           ▼

 Architecture Planning

           │

           ▼

 Backend Development

           │

           ▼

 AI Integration

           │

           ▼

 Database Design

           │

           ▼

 API Development

           │

           ▼

 Dockerization

           │

           ▼

 Cloud Deployment

           │

           ▼

 Continuous Improvement

```

---

# 🎯 Current Objectives (2026)

- 🚀 Build Enterprise AI Products

- 🤝 Contribute to Open Source

- 🧠 Master AI Agent Frameworks

- ☁️ Learn Cloud Native Systems

- ⚙️ Build Highly Scalable Backend Systems

- 📈 Strengthen System Design

- 💼 Secure an AI Backend Engineering Role

---

# 📖 Engineering Philosophy

> "Production-ready software is built through thoughtful architecture, clean code, automation, testing, and continuous improvement."

---

# 🔥 What You'll Find Here

✅ Production-ready Projects

✅ Enterprise AI Applications

✅ Backend Systems

✅ Cloud-native Development

✅ Open Source Contributions

✅ Technical Documentation

✅ System Design

✅ Continuous Learning

---
````

# 📊 GitHub Stats:
![](https://github-readme-stats.shion.dev/api?username=nick2726&theme=merko&hide_border=false&include_all_commits=true&count_private=true)<br/>
![](https://streak-stats.demolab.com/?user=nick2726&theme=merko&hide_border=false)<br/>
![](https://github-readme-stats.shion.dev/api/top-langs/?username=nick2726&theme=merko&hide_border=false&include_all_commits=true&count_private=true&layout=compact)

## 🏆 GitHub Trophies
![](https://github-profile-trophy.vercel.app/?username=nick2726&theme=radical&no-frame=false&no-bg=true&margin-w=4)

### ✍️ Random Dev Quote
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical)

### 🔝 Top Contributed Repo
![](https://github-contributor-stats.vercel.app/api?username=nick2726&limit=5&theme=dark&combine_all_yearly_contributions=true)

---
[![](https://komarev.com/ghpvc/?username=nick2726&icon=2&color=0)](https://visitcount.itsvg.in)

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->
